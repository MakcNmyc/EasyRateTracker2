...
class RefreshingTrackedRateTest {
    ...
    val networkObserverIdlingRule: NetworkObserverIdlingRule =
        ...
        { activity -> TrackedRateTest.getNetworkObserver(hiltFragmentRule, activity) }
    ...
    val recyclerViewIdlingRule: LoadElementInAdapterIdlingRule =
        ...
        { activity -> TrackedRateTest.getLoadElement(hiltFragmentRule, activity) }
    ...
    fun refreshingTrackedRateTest():Unit = runBlocking {
        Espresso.onView(withId(R.id.tracked_rates_list)).check(ViewAssertions.matches(matchSize(0)))
